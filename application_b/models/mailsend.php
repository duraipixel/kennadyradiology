...
function send_mail($tomail,$bccmail,$mlsubject,$bdymsg,$head='',$isbcc='',$mailfor='')
{
	//1 - order mail
	if($mailfor == 1){
	$bccmail = ",ecom@inbasetech.in";	
	}else {
	$bccmail = ",support@inbasetech.in";	
	}
	...
	$headers .= 'From: Inbase <support@inbase.com>' . "\r\n";
...
function send_mail_smtp($tomail,$bccmail='',$mlsubject,$bdymsg,$header='',$isbcc='')
{
	...
	if($isbcc == 1){
		$mail->addBCC('support@inbasetech.in');
	}else if($isbcc == 2){
		$mail->addBCC('ecom@inbasetech.in');
	}
	else{
		 ...
		 $helper->getStoreConfigvalue('ecomLogo');
				...
				<tr>
				   <td valign="top" style="background: #56514d;">
					<span style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:11px; font-weight:normal; line-height:18px; color:#fff;">&copy; Copyrights Reserved   '.date('Y').' Inbasetech </span>
					</td>					 
				</tr>
