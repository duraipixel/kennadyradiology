...
class common_model extends Model {
	...
	$str_all="select t1.* from ".TPLPrefix."banners t1 inner join  ".TPLPrefix."bannerposition t2 on  t1.Bannerposition=t2.bannaerposition and t2.bannaername='".$position."' and t2.IsActive =1  where  t1.IsActive=1 order by t1.SortingOrder asc "; 
	...
	function getorderdetails_history()
	{
        ...
        $str_all= " SELECT t1.*,t2.order_statusName as order_status,(case when t1.order_status_id in ('1','5') then  'Unsuccess'
			...
			'Success' end) as paymentstatus  FROM  `".TPLPrefix."orders` t1 inner join ".TPLPrefix."customers t3 on t3.customer_id = t1.customer_id left join ".TPLPrefix."order_status t2 on t2.order_statusId = t1.order_status_id where t1.customer_id='".$_SESSION['Cus_ID']."' and t1.IsActive=1 order by order_id desc  "; 			
	   // echo $str_all; exit;
	    $resulst=$this->get_rsltset($str_all);	
		return $resulst;
	}
