		...
		where adrp.base_productId=p.product_id  and cpa.cart_product_id = cp.cart_product_id and adrp.IsActive=1  and cpa.Attribute_id<>4 ) as other_attr_price , (SELECT  group_concat(concat(attr.attributename,' : ',dp.dropdown_values) SEPARATOR  '||') 
          ...
          AS attr_values, , (SELECT  group_concat(
           ...
           FROM ".TPLPrefix."product_attr_combi adrp
                ...
                inner join ".TPLPrefix."dropdown dp on dp.dropdown_id=cpa.Attribute_value_id      
                inner join ".TPLPrefix."m_attributes attr on attr.attributeid=cpa.Attribute_id      
				inner JOIN ib_product_images img
     ...
     function addtocartlist($cartpage='',$filters='')
	 {
		...
		//echo "<pre>"; print_r($prod_details); exit;
				...
				if(count($prod_details)>0){
					...
					$productlist=' <table class="table table-bordered table-striped">
						<thead>
							<tr>
					...
					$productlist.='<tr>
								<td class="text-center">
									...
									if($cartlist['IsCustomtool']==1){
										...
										/*if($cimgpath != ''){
											$productlist.='	<img src="'.BASE_URL.'uploads/productassest/'.$cartlist['product_id'].'/photos/'.$cimgpath.'" class="img-responsive" alt="'.$cartlist["product_name"].'"  /> ';
										}else*/ if($imgpath != ''){
									$productlist.='	<img src="'.BASE_URL.'uploads/productassest/'.$cartlist['product_id'].'/photos/'.$imgpath.'" class="img-responsive" alt="'.$cartlist["product_name"].'"  /> ';
										...
										$productlist.='	<img src="'.BASE_URL.'uploads/noimage/photos/noimage.png" class="img-responsive" alt="'.$cartlist["product_name"].'"  /> ';	
										}
									}		
									$productlist.='	</a>
										</span>
								...
								$productlist.='	 
									...
									$productlist.='	</h2>
									Qty: '.$cartlist['product_qty'].'
									<p class="product-price pt-2"><i class="fa fa-inr"></i> '.number_format(round($totaprice),2).'</p>
								...
								<input type="hidden" id="productid" value="'.$cartlist['cart_product_id'].'" >
									<button type="button" class="btn btn-danger btn-sm" onclick="deletecartfunction('.$cartlist['cart_product_id'].');">
		 ...
		 $productlist.='<div class="cartamount-wraper text-right">
						...
						$productlist.='
						<div class="cart-fixed-button">
						<div class="row">
							<div class="col-6 text-center">
								<a href="'.BASE_URL.'cart" class="common-btn">Go to Cart</a>
							</div>
							<div class="col-6 text-center">
								 <a href="'.BASE_URL.'checkout" class="common-btn white-btn">Checkout</a>
							</div>
					  </div>
					  </div>
...
/*		 if($_SESSION['Cus_ID']!='' && $_SESSION['cus_group_id']!=''){
	...
	function cartProductList($filters='',$orderjoinfields='',$orderjoinqry='',$isenableTax=0)
	{
		...
		$sortby=" order by cp.cart_product_id ";
		   ...
		   $strqry=$product->getProductQry($joinfields,$finddiscountqry,$joinfieldsafter,$joinqry,'',$conqry,$sortby,'',$groupby,'','',$imgpath_con_qry);	
