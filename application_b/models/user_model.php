		      ...
		      if(isset($filters['addressid']) && $filters['addressid']!=''){
				...
				foreach($rsltAdd as $displayaddress) {
                    if($filters['checkout']=='checkoutaddress'){
                       ...
                       $htmlappend .='<div class="selectaddress '.$selcls.' ">
										<div class="radiobtncss">
											<input type="radio" id="slctadd_'.$cnt.'" onChange="return displayshipping_add('.$displayaddress['cus_addressid'].')" name="slctadd" '.$chk.'> 
											...
											<label for="slctadd_'.$cnt.'" class="selsec">
												...
												<span class="seladddet selectaddr-wraper">
													<div class="seladddet-label">
														Address
													</div>
													<div class="seladddet-caption">
												...
												<div class="addrcommonbtn-wraper">
													<span>
													<a href="javascript:void(0);" class="addrcommon-btn small-lightbtn" onclick="javascript:editaddress('.$displayaddress['cus_addressid'].');" >Edit</a></span>
												</div>
											</label>
										</div>
										</div>';
				  ...
				  if($rsltMenu){
		       	...
		       	//Insert Address		
				$str="insert into ".TPLPrefix."cus_address (customer_id, firstname, lastname, address, emailid, city, postalcode, stateid, countryid,landmark,gstno,telephone, IsActive, UserId, CreatedDate, ModifiedDate)values('".$this->getRealescape($filters['customerid'])."','".$this->getRealescape($filters['firstname'])."','".$this->getRealescape($filters['lastname'])."','".$this->getRealescape($filters['address'])."','".$this->getRealescape($filters['email'])."','".$this->getRealescape($filters['city'])."','".$this->getRealescape($filters['zipcode'])."','".$this->getRealescape($filters['sel_state'])."','".$this->getRealescape($filters['sel_country'])."','".$this->getRealescape($filters['landmark'])."','".$this->getRealescape($filters['gstno'])."','".$this->getRealescape($filters['mobileno'])."','1','0','".$today."','".$today."')";		
                  //echo($str); exit;					
