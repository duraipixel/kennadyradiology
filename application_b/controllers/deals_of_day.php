<?php
class deals_of_day extends Controller {
	function index()
	{
			extract($_REQUEST);
			if(empty($scat) || $scat=='')
			...
			$fliterdetails=$product->displayfilter('',$catid,'','1','','12','1','deals_of_day');
			
			$productlists=$product->productlists('',$catid,'','1','','12','1','deals_of_day');	
			//$formfields=$common->getdynamicformfields('individual_dealer');
			//echo "<pre>"; print_r($formfields); exit;
			$SortBy=$product->getSortBy();
			$pageindex=1;
			$configmetatag = $common->common_metatag("config");
			$template = $this->loadView('deals_of_day');
			$headcss='<title>Deals_of_day-'.$configmetatag['title'].'</title>
