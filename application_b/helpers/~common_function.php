...
left join ".TPLPrefix."category_mobimage mi on mi.categoryID=c.categoryID and mi.IsActive = 1
 ...
 WHERE c.IsActive = 1 group by c.categoryCode HAVING pcnt > 0 order by c.sortingOrder,c.categoryID ");
...
function getMultiSelectBox_CustomFieds( $SelName, $Attr,$AttributeID,$selId=null) {	
	...
	$StrQry="select AttributeOptionId AS Id, AttributeValue AS Name from ".TPLPrefix."customfields_attrvalues where IsActive = '1' and AttributeId=? order by SortBy asc  ";	
	$resQry = $this->get_rsltset_bind($StrQry,array($AttributeID));		
	$strSelHtml =  "<select multiple='multiple' class='form-control select2".$Attr."' name='".$SelName.$AttributeID."[]' ><option value=''>Select</option>";
...
function getSelectBox_displaymastertable($AttributeName,$ValueCoumn,$ColumnName,$tablesname,$selId='1',$attr='',$class='',$afterscript,$additionalcond='') {	
 ...
 //   $cond=" and countryid='".$selId."' ";
	...
	$strSelHtml =  "<select data-placeholder='select' class='form-control select2 ".$class."' ".$attr." id='".$AttributeName."' name='".$AttributeName."' required='' >
...
function getRadioBox_FormFieds( $SelName, $Attr,$AttributeID,$selId=null) {
	...
	function displaymenu($activemenu='trophies-award')
	{
		...
		$subhtml='';
		$isdropdown='';
		$menuhtml='<ul class="nav navbar-nav menugroup-main">';
	/*	$menuhtml.='
		<li class="menumobileoption"><a href="'.BASE_URL.'">Home</a><span class="xsmenu-trigger" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"></span></a>
	</li>
	<li class="menumobileoption"><a href="'.BASE_URL.'aboutus">About Us</a><span class="xsmenu-trigger" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"></span></a>
	</li>
		<li class="menumobileoption"><a href="'.BASE_URL.'media">Media</a><span class="xsmenu-trigger" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"></span></a>
	</li> 
		...
		foreach($rslt_getfrontmenus as $rslt_getfrontmenus_S){
			...
			switch($rslt_getfrontmenus_S['f_menutype'])
			{
			   ...
			   case "3":
							...
							if($activemenu==$catinfo['categoryCode'])
								$isactive=' active';
							if(count($subcat)>0)
							{
								$isdropdown='dropdown';
								...
								$menulink='<a href="'.BASE_URL.$catinfo['categoryCode'].'" class="dropdown-toggle" >';
								foreach($subcat as $sub)
								{
									...
									<div class="category-title"><a data-img="'.$baseimgurl.'" href="'.BASE_URL.$catinfo['categoryCode'].'/'.$sub['categoryCode'].'">'.$sub['categoryName'].'</a></div>';
									...
									{   $subhtml.='<ul class="customdropdown">';
										foreach($subcatplus as $plus)
										{
											...
											$subhtml.='<li>
														<a href="'.BASE_URL.$catinfo['categoryCode'].'/'.$sub['categoryCode'].'/'.$plus['categoryCode'].'" class="menuitem" data-img="'.$baseimgurl.'">'.$plus['categoryName'].'</a>
														</li>';
								...
								$subhtml.='</div>
														...
														<div class="hoverimg-wraper">
															<div class="hoverimg-outer">
																...
																<span>'.$catinfo['categoryName'].'</span>
																
																</div>
																<div>
																<img src="'.BASE_URL.'static/images/noimage.jpg" class="img-responsive" id="menuimage" alt="'.$catinfo['categoryName'].'">
															</div>
															</div>
														</div>
													</div>
								...
								$menulink='<a href="'.BASE_URL.$catinfo['categoryCode'].'">';
			   ...
			   case "4":														
							...
							if (strpos($rslt_getfrontmenus_S['f_link_name'], 'http://') === false) {
								...
								if($rslt_getfrontmenus_S['f_link_name']=="home")
									...
									$menulink='<a href="'.BASE_URL.$rslt_getfrontmenus_S['f_link_name'].'">';
							}
							else
							{
								$menulink='<a href="'.$rslt_getfrontmenus_S['f_link_name'].'">';
							}
							break;
			}
			$clshidemobile='';
	...
	function displayproductsilder($catid='',$type='',$title='',$viewall='',$limit='10',$productid='',$homeslidertitle='',$addclass='')
	{
		
		$product=$this->loadModel('product_model');
		$products=$product->productlists('',$catid,'','1','',$limit,'0',$type,$productid,$homeslidertitle);
	...
	function displayhomeslider($startpostion='',$endpostion='')
	{
	  $conqry='';
		...
		$StrQry="select title,categoryid,hpsid,type  from ".TPLPrefix."homepageslider where IsActive=1  ". $conqry." order by sortby asc ";	
	...
	function displayhomesCategorylider($startpostion='',$endpostion='',$title='',$viewall='',$limit='10',$productid='',$homeslidertitle='',$addclass='' )
	{
	  $conqry='';
		 ...
		 $StrQry="select cs.title,(case when cs.type=0 then  hc.categoryID else cs.categoryid end ) as categoryid,cs.hpsid,cs.type  from ".TPLPrefix."homepagecatslider cs
		left join ".TPLPrefix."homepageslider_category hc on hc.hpsid=cs.hpsid and hc.IsActive=1
		where cs.IsActive=1 and  (case when cs.type=0 then hc.hpd_proid is not null else cs.categoryid is not null end ) ". $conqry."
		   group by   (case when cs.type=0 then  hc.categoryID else cs.categoryid  end )
		order by cs.sortby asc ";	
