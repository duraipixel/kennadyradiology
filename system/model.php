...
class Model {
	...
	public function __construct()
	{
		...
		/*	print_r('jjj'); die();
			...
			$this->pdo = new ezSQL_mysqli( $config['db_username'],$config['db_password'],$config['db_name'], $config['db_host'],$config['dbport']);
	...
	public function get_a_line_bind($query=null, $param=null,$output = ARRAY_A,$y=0)
	{
			...
			if ( $query )
			{
				...
				if($param!=null){
					// print_r($query);
				$this->pdo->query_prepared($query,$param);
				//$this->pdo->query($query);
	...
	function get_rsltset_bind1($query=null, $param=null,$output = ARRAY_A)
	{
			 $this->pdo->flush();   
			// If there is a query then perform it if not then use cached results..
			if ( $query )
			{
				...
				if($param!=null){
				//	 print_r("jj");
				$this->pdo->query_prepared1($query,$param);
				//$this->pdo->query($query);
				//$this->pdo->query_prepared('SELECT * FROM kesar_banners WHERE bannerid in (?,?) ',[1,2] );
				//$result = $this->pdo->last_result;
				//print_r($result );
	...
	{	
		$escape = 	$this->pdo->escape($data);
	//	$escape = mysqli_real_escape_string($this->pdo,$data);
		return $escape;
	}
