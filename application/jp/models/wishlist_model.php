...
class wishlist_model extends Model {
	...
	function WishllistProductList($filters='')
	{
		...
		where adrp.base_productId=p.product_id  and cpa.cart_product_id = cp.cart_product_id and adrp.IsActive=1 and cpa.Attribute_id=4  and  adrp.outofstock = 0 ) as attr_price , @other_attr_price:=( select sum(adrp.price)  from ".TPLPrefix."product_attr_combi adrp inner join ".TPLPrefix."wishlist_products_attribute cpa on find_in_set(cpa.Attribute_value_id, REPLACE(adrp.attr_combi_id,'_',',')) and cpa.isactive=1
		where adrp.base_productId=p.product_id  and cpa.cart_product_id = cp.cart_product_id and adrp.IsActive=1 and cpa.Attribute_id<>4  and  adrp.outofstock = 0 ) as other_attr_price , (SELECT  group_concat(concat(attr.attributename,' : ',dp.dropdown_values) SEPARATOR  '||') 
