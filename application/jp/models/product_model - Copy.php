...
//	print_r($arrparams); die();
	...
	function getSortBy()
	{
		$strqry= " SELECT SortId,SortName from ".TPLPrefix."sortby where IsActive=1 order by SortId asc ";
	...
	function getProductQry($joinfields='',$finddiscountqry='',$joinfieldsafter='',$joinqry='',$attrjoinqry='',$conqry='',$sortby='',$limitqry='', $groupby='',$type='',$selectqry='')
	{
		...
		*/
		...
		if($selectqry=='')
		{
		 ...
		 $strqry=" select p.product_id,p.product_name,p.description,p.sku,p.product_url,p.price,p.specialprice,p.spl_fromdate,p.spl_todate,p.isnewproduct,p.newprod_fromdate,p.newprod_todate,p.soldout,p.minquantity,pc.categoryID,cat.hsncode, t.taxTyp,t.taxRate, d_prod.DiscountType  as prod_DiscountType,d_prod.DiscountAmount  as prod_DiscountAmount ,d_cat.DiscountType as   cat_DiscountType,d_cat.DiscountAmount as cat_DiscountAmount,d_deals.DiscountType  as deals_DiscountType,d_deals.DiscountAmount  as deals_DiscountAmount,group_concat(DISTINCT img.img_path
        ORDER BY img.product_img_id ASC  SEPARATOR '|') as img_names ";
		...
		left join ".TPLPrefix."product_images img on img.product_id=p.product_id and img.IsActive=1 and img.ordering in(1,2) left join ".TPLPrefix."discount d_prod on find_in_set(p.product_id,d_prod.DiscountProducts) and d_prod.IsActive=1 and '".date('Y-m-d')."' between d_prod.DiscountStartDate and d_prod.DiscountEndDate 
