<?php
class bulkorder extends Controller {
	function index()
	{
		...
		$bulk_order_form=$common->getdynamicformfields_bulk_equiry('bulk_order_enquiry');
		//print_r($bulk_order_form); exit;
	 	$template = $this->loadView('bulkorder_view');
		
		$headcss='<title>'.$configmetatag['title'].' Bulkorder</title>
