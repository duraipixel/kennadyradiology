        ...
        <div class="row align-items-center">
            <div class="col-sm-12 col-md-4 col-lg-6">
                <div class="show-on-scroll">
                    ...
                    <div id="detailspricewrapers" class="detailsprice-wraper">
                        <?php if($productdetails['totpent']>0){ ?>
                        <p class="offerspan"><?php echo $productdetails['totpent']; ?>%</p>
                        <h5><s><?php echo PRICE_SYMBOL;?>&nbsp;<?php echo number_format($productdetails['final_price_tax'],2);  ?></s>
                        </h5>

                        <h5><?php echo PRICE_SYMBOL. number_format($productdetails['final_price_tax'],2); ?></h5>
                        <?php }ELSE {?>
                        ...
                        <?php }; ?>
                        <?php }?>
                        <?php if($productdetails['isbuynow'] == 0){?>
                    </div><?php }?>
                </div>
            </div>
            <div class="col-sm-12 col-md-8 col-lg-6 text-right">
                ...
                <button type="button" class="add-to-cart-btn1"
                    onClick="addtocart('<?php echo $productdetails['product_id'];?>')" ;>
                    <?php echo $commondisplaylanguage['addtocart'];?> <i class="flaticon-cart-bag"></i>
                </button>
                <button type="button" class="buy-now-btn1"
                    onClick="buynow('<?php echo $productdetails['product_id'];?>')" ;>
                ...
                <span id="detailspricewraper" class="detail-price">
                    ...
                    <?php if($productdetails['totpent']>0){ ?>
                    <?php echo $productdetails['totpent']; ?>%
                    <s> <?php echo PRICE_SYMBOL;?>&nbsp;<?php echo number_format($productdetails['final_price_tax'],2);  ?>
                    </s>

                    <?php echo PRICE_SYMBOL. number_format($productdetails['final_price_tax'],2); ?>
                    <?php }ELSE {?>
                    <?php echo PRICE_SYMBOL.'&nbsp;'. number_format($productdetails['final_price_tax'],2);  ?>
                    <?php }; ?>
                    <?php }ELSE { ?>
                    ...
                    <?php }; ?>
                    <?php }?>
                    <?php if($productdetails['isbuynow'] == 0){?> <?php }?>
                </span>
            </div>
        </div>
    </div>
        ...
        <div class="row">
                ...
                <div id="productimage">
                    <ul id="glasscase" class="gc-start">
						...
						if($productdetails['img_names']!=''){
							...
							?>
                        <li><img src="<?php echo img_base;?>uploads/productassest/<?php echo $productdetails['product_id']; ?>/photos/base/<?php echo $list_img; ?>"
                                alt="Text" /></li>
                        <?php 
							}
						}else{ ?>
            ...
            <div class="col-sm-12 col-md-12 col-lg-6">
                <div class="pad-lef-30">
                    <h1 class="product-name"><?php echo $productdetails['product_name']; ?> <strong
                            class="sku"><?php echo $commondisplaylanguage['sku'];?>#<span
                                id="spnsku"><?php echo $productdetails['sku']; ?><span></strong></h1>
                    <?php 
					if($productdetails['isbuynow'] == 0){
					if($productdetails['final_price_tax']>0){ 
					if($productdetails['soldout']==0) { ?>
                    ...
                    <?php  
								if($productsizechart['attribute_value'] != ''){?><p><a href="javascript:void(0)" data-toggle="modal"
                    ...
                    <div class="row">
                        <div class="col-sm-12 col-md-12 col-lg-12">
                            ...
                            <div class="input-group quantity-buttons">
                                ...
                                <input type="text" class="form-control input-number chkqtydetail"
                                    id="prices1_<?php echo $productdetails['product_id']; ?>"
                                    min="<?php echo $productdetails['minquantity']; ?>" onChange="" onMouseMove=""
                                    onKeyPress="return validateQty(event);" onKeyDown="numberkeyvalid(event);"
                                    onBlur="chkqtydetail()" max="100"
                                    value="<?php echo $productdetails['minquantity']; ?>">
...
function prodattrchange(aid, sku, did, attrcode, clickedind) {
    ...
    data = "proid=<?php echo $productdetails['product_url']; ?>&sku=" + tsku + "&clickedind=" + clickedind + "&" + $(
        "#frmcustomattr").serialize();
    //console.log(path);
    // location.href=path;
