   ...
   <div class="container">
      <div class="row">
         <div class="col">
			...
			<h1 class="heading1 text-center text-white"> Events</h1>
		 </div>
	  </div>
	</div>
</section> 
...
<section class="light-gray-bg border-bottom my-account">
   <div class="container">
	...
	foreach($newsList as $newsvals){?>
				 <a href="<?php echo BASE_URL; ?>events/<?php echo $newsvals['eventcode'];?>" class="features-stories-link">
					<div class="date"><strong><?php echo date('d',strtotime($newsvals['eventdate']));?></strong> <?php echo date('M Y',strtotime($newsvals['eventdate']));?></div>
					<h4><?php echo $newsvals['eventtitle'];?></h4>
					<p><?php echo substr(strip_tags($newsvals['eventdescription']),0,300); if(strlen($newsvals['eventdescription']) > 300){ echo '...';} ?></p> 
					<span>Read More <i class="fa fa-angle-right" aria-hidden="true"></i></span>
				</a>
				<?php }?>
				</span>  <div class="newscontent"></div>
				<?php }else{?>
...
<script>
	...
	function ChangeyearNews(year,month){
			...
			method     : 'POST',
			dataType   : 'json',
			url: "<?php echo BASE_URL; ?>ajax/getEventList_ajax",
			data       : "year="+year+'&month='+month+'&action=filter&page='+page,
			beforeSend: function() { $("#newspage").val(2);	loading()},	
			cache: false,			
			success: function(data){ 
				...
				//	var bb = data.maincount / <?php echo $getpagecount;?>;
			 	...
			 	//if(page>=bb.toFixed() || data.maincount == '' || data.oppcnt == 0){	
