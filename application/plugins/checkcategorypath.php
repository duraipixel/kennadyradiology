...
class checkcategorypath extends Model {
	...
	{
		$allcat = $this->get_rsltset("SELECT c.categoryID,c.categoryCode FROM ".TPLPrefix."category c  where c.lang_id = ".$_SESSION['lang_id']." group by c.categoryCode order by c.sortingOrder,c.categoryID ");	
	...
	function getStoreConfigPlugin()
	{
        ...
        ORDER BY i.ordering ASC  SEPARATOR '|') as catimage FROM ".TPLPrefix."category c left join ".TPLPrefix."categoryimage i on i.categoryID=c.categoryID and i.IsActive = 1 WHERE c.IsActive = 1 and c.lang_id = ".$_SESSION['lang_id']." group by c.categoryCode order by c.sortingOrder,c.categoryID ");
