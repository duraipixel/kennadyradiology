...
switch($act)
{
	...
	if(!empty($shippingCost) ) {
		...
		if($reslt[0] == 0) {
			  ...
			  $strmain="insert into ".TPLPrefix."shipping_flat(`shippingId`,pricetype,`shippingTitle`,`orderMinimum`,`shippingCost`, `customer_group_id`,  `IsActive`, `createdDate`, `UserId`, `sortingOrder`,countryid,stateid) values('".$shippingId."','".$pricetype."','".$shippingTitle."','0','".$shippingCost."','".$customer_group_id."','".$status."','".$today."','".$_SESSION["UserId"]."','".$txtSortingorder."','".$cntyid."','".$sttid."')";
			$rslt = $db->insert($strmain);	 
			
			$logmain = $db->insert_log("insert","".TPLPrefix."shipping_flat","","Flat Shipping Rate Added Newly","shipping",$strmain);
	...
	$strChk = "select count(flatshippingId) from ".TPLPrefix."shipping_flat where shippingCost = '$shippingCost' and IsActive != '2' and flatshippingId != '".$edit_id."' ";
 		$reslt = $db->get_a_line($strChk);
		if($reslt[0] == 0) {
			$str = "update ".TPLPrefix."shipping_flat set orderMinimum = '".$orderMinimum."', pricetype = '".$pricetype."' , shippingTitle = '".$shippingTitle."', shippingCost = '".$shippingCost."', customer_group_id = '".$customer_group_id."', IsActive = '".$status."', countryid = '".$cntyid."',stateid = '".$sttid."', modifiedDate = '$today' , UserId='".$_SESSION["UserId"]."'  where flatshippingId = '".$edit_id."'";
			$db->insert($str);
			$db->insert_log("update","".TPLPrefix."shipping_flat",$edit_id,"Flat Shipping Rate updated","shipping",$str);
