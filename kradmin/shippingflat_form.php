...
$mdme = getMdmeShippingmodules($db,''); //getMdmeFlatShipping
        ...
        <div class="page-title">
          ...
          <div class="crumbs">
            <ul id="breadcrumbs" class="breadcrumb">
              ...
              <li><a href="shipping_mng.php">Shipping Modules</a></li>
              <li class="active"><a href="#"><?php echo $operation; ?> Flat Rate Shipping</a> </li>
            </ul>
          </div>
        </div>
      </div>
            ...
            <div class="widget-header">
              <div class="row">
                <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                  <h4><?php echo $operation; ?> Flat Rate Shipping</h4>
                </div>
              </div>
            </div>
                      ...
                      <div class="col col-md-6">
                        <div class="control-group mb-4">
                          <div class="controls">
                            ...
                            <button class="btn btn-warning btn-rounded snackbar-txt-warning mb-4" type="button" onClick="javascript:funSubmt('frmShipFlat','shippingflat_actions.php','jvalidate','shippingflat','shippingflat_mng.php?shipping=MQ==');"><span id="spSubmit"><?php echo $btn; ?></span></button>
                              <button class="btn btn-dark btn-rounded snackbar-bg-dark mb-4" type="reset" onClick="javascript:funCancel('frmShipFlat','jvalidate','shipping','shippingflat_mng.php?shipping=MQ==');">Cancel</button>
                          </div>
                        </div>
                      </div>
                    </div>
...
<script>
	...
	{	
		...
		$.ajax({
			...
			type: 'POST',
			dataType : 'json',
			data: 'action=shipping_flat&countryid='+$cntid,
			beforeSend: loading(),
			success: function(response) {	 														
				unloading();
				$("#statediv").html(response.rslt);
				$('.select2').select2();
			}
		}); 	
	});
	
function chkPriceTyp()
{
	var ctyp = $("#pricetype").val();
	var camt = $("#shippingCost").val();
	
	if(ctyp == 1){ $("#shippingCost").attr("placeholder", "%"); }
	if(ctyp == 2){ $("#shippingCost").attr("placeholder", "Amount"); }
	if(ctyp == ''){ $("#shippingCost").attr("placeholder", "Pricetype"); }
	if( (ctyp == 1) && (camt >=100))
	{
	$("#shippingCost").val('100');
