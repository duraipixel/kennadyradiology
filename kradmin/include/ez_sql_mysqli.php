<?php
	...
	*/
	...
	class ezSQL_mysqli extends ezSQLcore
	{
		...
		*/
		...
		function ezSQL_mysqli($dbuser='', $dbpassword='', $dbname='', $dbhost='localhost', $dbport='3306', $encoding='')
		{
			$this->dbuser = $dbuser;
		...
		*/
		...
		function insert_bind($query=null, $param=null)
		{
			...
			{
				$is_insert = true;
				$this->rows_affected = @$this->affected_rows;
    ...
    private function fetch_prepared_result(&$stmt, $query)
    {
		...
		function multi_query($query)
		{
			...
			$this->result = @$this->dbh->multi_query($query);
    ...
    public function query_prepared(string $query, array $param = null)
    {
		...
		//	print_r($this->dbh); die();
		//print_r($this->dbh->prepare($query)); die();
		//$this->dbh->prepareOn(); 
  ...
  public function query_prepared2(string $query, array $param = null)
    {
		if ( ! isset($this->dbh) || ! $this->dbh )
			{
				$this->connect($this->dbuser, $this->dbpassword, $this->dbhost, $this->dbport);
		...
		if(!$result)
		{
			...
			$this->register_error($stmt->error);
			
			$this->show_errors ? trigger_error($stmt->error,E_USER_WARNING) : null;
