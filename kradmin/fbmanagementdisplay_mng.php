<?php 
$attributesdisp = "fbmanagementdisplay";
include "includes/header.php"; 
include "includes/Mdme-functions.php";
 $tablename=$_REQUEST['tablename'];
 
 //echo $tablename; exit;
$mdme = getMdmefbmanagementdisplay($db,'fbmanagementdisplay_mng.php?tablename='.$tablename);
 
$str_all="select t2.*,t3.elementid,t3.elementname,t3.element_type from ".TPLPrefix."formbuilder t1 inner join ".TPLPrefix."fb_attributes t2 on t1.FormId=t2.FormId and t2.IsActive=1 inner join ".TPLPrefix."m_elements t3 on t3.elementid=t2.AttributeType and t3.IsActive=1 where t1.fromtablename='".$tablename."' group by t2.AttributeCode order by sortBy asc  ";

$res_eds = $db->get_rsltset($str_all);
//print_r($res_eds); exit;

include_once "includes/pagepermission.php";?>
<?php include "includes/top.php";?>

<!--  BEGIN MAIN CONTAINER  -->
<div class="main-container" id="container">
  <div class="overlay"></div>
  <div class="cs-overlay"></div>
  
  <!--  BEGIN SIDEBAR  -->
  
  <?php include "includes/sidebar.php";?>
  
  <!--  END SIDEBAR  --> 
  
  <!--  BEGIN CONTENT PART  -->
  <div id="content" class="main-content">
    <div class="container">
      <div class="page-header">
        <div class="page-title">
          <h3>FB Display </h3>
          <div class="crumbs">
            <ul id="breadcrumbs" class="breadcrumb">
              <li><a href="dashboard.php"><i class="flaticon-home-fill"></i></a></li>
              <li><a href="#">Enquiry</a></li>
              <li class="active"><a href="#">FB Display </a> </li>
            </ul>
          </div>
        </div>
      </div>
      <div class="row layout-spacing">
        <div class="col-lg-12">
          <div class="statbox widget box box-shadow">
            <div class="widget-header">
              <div class="row">
                
                  <div class="col-md-8"><h4>Manage FB Display  </h4></div>
                  
                
              </div>
            </div>
            <div class="widget-content widget-content-area">
              <div class="table-responsive mb-4">
             <input type="hidden" name="disptblname" id="disptblname" value="<?php echo "fbmanagementdisplay"; ?>" />
		    <input type="hidden" name="tablename" id="tablename" value="<?php echo $tablename; ?>" />
                <table id="tblresult" class="table table-striped table-bordered table-hover">
                  <thead>
                    <tr>
                      <?php foreach($res_eds as $dynamicdata){ ?>
						<th><?php echo $dynamicdata['AttributeCode']; ?></th>
						<?php } ?>
                    </tr>
                  </thead>               
                </table>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
  <!--  END CONTENT PART  --> 
</div>
<!-- END MAIN CONTAINER --> 

<!--  BEGIN FOOTER  -->
<?php include('includes/footer.php');?>
<!--  END FOOTER  -->

<script>
      $(function () {       
		  datatblCal(dataGridHdn);
	  });
</script>