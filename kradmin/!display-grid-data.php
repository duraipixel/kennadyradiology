...
$data = array();												
	...
	{		
		...
		if($_REQUEST['finaltab']=='product'){
			$r['img_names'] = '<a class="product-list-img" href="javascript: void(0);"><img src="'.IMG_BASE_URL.'productassest/'.$r['product_id'].'/photos/'.$r['img_names'].'" alt="product"></a>';
		}
...
if($_REQUEST['finaltab'] == "relatedProducts"){
	...
	if( $_REQUEST['finaltab'] == "orders" && $r["order_status_id"]!=14){
		...
		$incstat ='<select class="form-control orders_mangement select2" data-orderid="'.$r["order_id"].'"  name="status">';
...
";
    ...
    if($_REQUEST['finaltab']=='product'){
		 ...
		 if($ishome == 0){ 
		  ...
		  $incstat_show =  ' 		
							<span class="badge badge-danger" onclick="funchangestatus(this,'.$statusurlc.');"> <i class="fa fa-circle"></i> HIDE </span>
				 	 	  ';
		}else{
		  $statusurlc = "'product_actions.php','Id=$editid&action=changeishome&actval=0'";			
		  $incstat_show =  ' 	
							<span class="badge badge-success" onclick="funchangestatus(this,'.$statusurlc.');"> <i class="fa fa-circle"></i> SHOW </span>
				 		 ';								 					 
		}
		$nestedData[] = $incstat_show;
		if($r['IsActive'] == '1')
		{
			$incstat1 =  '<i class="flaticon-cart-bag-1" data-toggle="tooltip" title="Approved"></i>'	;	
		}
        else
		{
			$incstat1 =  '<i class="flaticon-cart-bag" data-toggle="tooltip" title="Pending Approved"></i>';
		}
		$nestedData[] = $incstat1;
