...
{ 
		 ...
		 foreach($getlanguage as $languageval){ 
			...
			foreach($_REQUEST as $key => $val){
				if(strpos($key,"combiqua")  !== false){
					...
					$strCombiChk = "select count(*) as tot from ".TPLPrefix."product_attr_combi where attr_combi_id = '$combId' and base_productId = '$lastInserId' ";
					...
					if($resltCombi['tot'] == 0){						 						
						...
						$combiStr .= " VALUES ('".$combId."','".$lastInserId."','".$qua."','".$price."','".$sku."','".$default."','".$today."','".$today."','1','".$combimgids."') ";
						$db->insert($combiStr);
						 $log = $db->insert_log("insert","".TPLPrefix."product_attr_combi","","product_attr_combi Add successfully","product_attr_combi",$combiStr);
					}
					else{
						$combiStr = "UPDATE ".TPLPrefix."product_attr_combi SET  product_img_id='".$combimgids."',quantity= '".$qua."',price = '".$price."', sku = '".$sku."', modifiedDate='".$today."',isDefault =  '".$default."',IsActive='1' WHERE  base_productId = '".$lastInserId."' AND  attr_combi_id =  '".$combId."'  ";
			...
			$strCombiOptChk = "select count(*) as tot from ".TPLPrefix."product_attr_combi_opt where product_id = '$edit_id' ";
		    $resltCombiChk = $db->get_a_line($strCombiOptChk);
			if($resltCombiChk['tot'] == 0){
				$str = "INSERT INTO ".TPLPrefix."product_attr_combi_opt(optionId,product_id,createdDate,modifiedDate) VALUES('".$optionRes['attrIds']."','".$edit_id."','".$today."','".$today."')";
				...
				$str = "UPDATE ".TPLPrefix."product_attr_combi_opt SET optionId = '".$optionRes['attrIds']."',modifiedDate='".$today."' WHERE product_id = '".$edit_id."' ";
			...
			if(isset($_REQUEST["categoryIDs"])){
				foreach($categoryIDs as $val){
					...
					$str = "insert into ".TPLPrefix."product_categoryid(product_id,categoryID,IsActive,UserId,createdDate,modifiedDate,lang_id) values('".$splastInserId."','".$getcategoryid_es['categoryID']."',1,'".$_SESSION["UserId"]."','".$today."','".$today."',3) ";						
					$db->insert($str);
	...
	if(!empty($product_name)) {	
		...
		if($isnewproduct==1){	 
			...
			$str .= " soldout='".getRealescape($soldout)."',chkpvatt='".getRealescape($priceatt)."' , UserId='".$_SESSION["UserId"]."', related_products='".$related_products."',suggested_products='".$suggested_products."',modified_date = '".$today."'  where product_id = '".$edit_id."'";
			...
			$strs .= " soldout='".getRealescape($soldout)."',chkpvatt='".getRealescape($priceatt)."' , UserId='".$_SESSION["UserId"]."', related_products='".$related_products_es."',suggested_products='".$suggested_products_es."',modified_date = '".$today."'  where product_id = '".$edit_id_es."'";		   		
			$db->insert($strs);
			...
			$strs .= " soldout='".getRealescape($soldout)."',chkpvatt='".getRealescape($priceatt)."' , UserId='".$_SESSION["UserId"]."', related_products='".$related_products_pt."',suggested_products='".$suggested_products_pt."',modified_date = '".$today."'  where product_id = '".$edit_id_pt."'";		   		
			$db->insert($strs);
			...
			foreach($_REQUEST as $key => $val){
				if(strpos($key,"combiqua")  !== false){
					...
					$strCombiChk = "select count(*) as tot from ".TPLPrefix."product_attr_combi where attr_combi_id = '$combId' and base_productId = '$edit_id' ";
					$resltCombi = $db->get_a_line($strCombiChk);
					if($resltCombi['tot'] == 0){
						$combiStr = "INSERT INTO ".TPLPrefix."product_attr_combi(attr_combi_id,base_productId,quantity,price,sku,isDefault,createdDate,modifiedDate,IsActive,product_img_id) ";
						$combiStr .= " VALUES ('".$combId."','".$edit_id."','".$qua."','".$price."','".$sku."','".$default."','".$today."','".$today."','1','".$combimgids."') ";
						$db->insert($combiStr);
						$log = $db->insert_log("insert","".TPLPrefix."product_attr_combi","","product_attr_combi inserted","product_attr_combi",$combiStr);
					}
					else{
						$combiStr = "UPDATE ".TPLPrefix."product_attr_combi SET product_img_id='".$combimgids."',quantity= '".$qua."',price = '".$price."', sku = '".$sku."', isDefault =  '".$default."',modifiedDate = '".$today."',IsActive='1' WHERE  base_productId = '".$edit_id."' AND  attr_combi_id =  '".$combId."'  ";
...
function uploadPortfolio_bck($edit_id,$db,$sku){
	//echo $sku; die();
$today=date("Y-m-d H:i:s");	
$configinfo=getQuerys($db,"allconfigurable");
$getsize = getimagesize_large($db,'product','product_image');
		...
		if(isset($_FILES['product_images'])){	
			...
			{			
				...
				for($i=0;$i<count($_FILES["product_images"]["name"]); $i++){
					...
					echo $_FILES["product_images"]["tmp_name"][$i];
					echo "\n\n";
						echo $target_file;
				//	if(move_uploaded_file($_FILES["product_images"]["tmp_name"][$i], $target_file));	
					if(file_exists( $target_file))	
